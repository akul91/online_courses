...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda x:('xyz',pow(x[1][0]-x[1][1],2))))
    
    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.reduceByKey(lambda a,b:a+b).collectAsMap()

    # Count the number of entries for which you computed the total squared error
    numRatings = squaredErrorsRDD.count()

    # Using the total squared error and the number of entries, compute the RSME
    return math.sqrt(totalError['xyz']/float(numRatings))
...
trainingAvgRating = trainingRDD.map(lambda x:(1,x[2])).reduceByKey(lambda a,b:a+b).collect()[0][1]/trainingRDD.count()
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .filter(lambda x:x[1][0]>75).map(lambda x:(x[1][0],x[0],x[1][1])))
