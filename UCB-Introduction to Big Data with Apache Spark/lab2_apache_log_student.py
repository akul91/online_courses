...
topTenErrURLs = endpointSum.takeOrdered(10, lambda log: -log[1]) 
...
dayHostCount = dayGroupedHosts.mapValues(lambda x: list(set(x)))

dailyHosts = (dayHostCount
             .map(lambda x: (x[0],len(x[1]))).sortByKey(True).cache())
...
groupedByDay = dayAndHostTuple.groupByKey().mapValues(lambda x: list(x)).map(lambda x: (x[0],len(x[1])))

sortedByDay = groupedByDay.sortByKey(True)

avgDailyReqPerHost = (sortedByDay
                      .leftOuterJoin(dailyHosts).sortByKey(True).map(lambda x: (x[0],x[1][0]/x[1][1]))).cache()
...
badEndpoints = badRecords.map(lambda x:(x.endpoint,1)).reduceByKey(lambda a, b : a + b)

badUniqueEndpoints = badEndpoints.filter(lambda x: x[1]==1)
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20, lambda x: -x[1])
...
errHostsTop25 = errHostsSum.takeOrdered(25, lambda x: -x[1])
...
topErrDate = errDateSorted.takeOrdered(5,lambda x: -x[1])
...
hoursWithErrors404 = hourRecordsSorted.map(lambda (a,b):a).collect()
errors404ByHours = hourRecordsSorted.map(lambda (a,b):b).collect()
