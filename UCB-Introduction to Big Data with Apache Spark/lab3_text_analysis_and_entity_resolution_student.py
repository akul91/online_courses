...
def tokenize(string):
    ...
    """
    return [x for x in filter(None,re.split(split_regex,string.lower())) if x not in stopwords]
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda x: ('xyz',len(x[1]))).reduceByKey(lambda a,b:a+b).collect()[0][1]
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.flatMap(lambda (a,b):set(b))
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict ={k : v * idfs[k] for k, v in tfs.items() if k in idfs}
    return tfIdfDict
...
avgSimDups = sum(trueDupsRDD.map(lambda x: x[1][0]).collect())/float(trueDupsCount)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)).filter(lambda x:x[1][1] is None)
avgSimNon = sum(nonDupsRDD.map(lambda x: x[1][0]).collect())/float(nonDupsRDD.count())
...
commonTokens = (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(swap).reduceByKey(lambda a,b: a)
                .cache())
